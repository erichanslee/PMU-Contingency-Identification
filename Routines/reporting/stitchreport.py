...
## Plot Eigenvalue Shifts
filename = 'eigenvaluesPlot.jpeg'
parts.append(Paragraph("Eigenvalue Shifts", styles['Heading1']))
parts.append(Image(filename, width=8.75*cm, height=6.56*cm))

## Plot Eigenvector Angle Shifts
jpegCounter = len(glob.glob1(os.getcwd() ,"angleplot*.jpeg"))
parts.append(Paragraph("Eigenvector Angle Shifts", styles['Heading1']))
for i in range(1, jpegCounter + 1):
	filename = 'angleplot%d.jpeg' % i
	parts.append(Image(filename, width=8.75*cm, height=6.56*cm))	
...
jpegCounter = len(glob.glob1(os.getcwd() ,"fittingerror*.jpeg"))
